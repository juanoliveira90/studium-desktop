...
src/                    React frontend
  ...
  data/                 mock.ts — placeholder data for the not-yet-vault-backed pages (plans, home) + derivation helpers
...
- The notes page (step 3) and schedule page (step 4) read the real vault via TanStack Query over the `src/vault/ipc.ts` invoke layer; components never call `@tauri-apps/api` directly, and tests mock `ipc.ts` instead of the webview. Plans/home pages are still static placeholder data from `src/data/mock.ts` until steps 5–6 land.
